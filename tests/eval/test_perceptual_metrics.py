...
def test_cosine_and_spatial_lpips_roi_mean_are_correct():
    ...
    assert lpips_scores(torch.tensor([[[[9.0]]]]), spatial, mask) == {"full_lpips": pytest.approx(9.0), "roi_lpips": pytest.approx(2.5)}


def test_full_lpips_and_roi_lpips_use_distinct_reductions():
    spatial = torch.tensor([[[[1.0, 5.0], [9.0, 13.0]]]])
    mask = np.array([[True, False], [False, False]])
    assert lpips_scores(torch.tensor([[[[7.0]]]]), spatial, mask) == {"full_lpips": pytest.approx(7.0), "roi_lpips": pytest.approx(1.0)}
