...
class FinetuneSolverBase(ABC):
    ...
    def __init__(self, args):
        ...
        # =======================================================
        # build model & resume
        # retuen --> model, text tokenizer, optimizer
        # =======================================================
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument("--accum_iter", default=4, type=int, help="Accumulate gradient iterations " "(for increasing the effective batch size under memory constraints)",)
        ...
        parser.add_argument("--min_lr", type=float, default=0.00, help="lower lr bound for cyclic schedulers that hit 0")
        ...
        parser.add_argument("--cache_ann_on_disk", action="store_true", help="cache the dataset annotations on disk to avoid duplication across ranks. can save CPU memory, especially with large datasets",)
        parser.add_argument("--length_clustering", default=True, help="gather items with similar length to the same batch",)
        ...
        parser.add_argument("--save_iteration_interval", default=5000, type=int, help="number of iterations between within-epoch model saving",)
        parser.add_argument("--only_save_trainable", default=False, action="store_true", help="only save trainable model parameters")
        parser.add_argument("--ckpt_max_keep", default=2, type=int, help="maximum number of checkpoints to keep, <=0 means keep all")
        ...
        # parser.add_argument('--quant', action="store_true", default=False, help="enable quantization to speedup and save memory") # todo
    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        # =======================================================
        # random init --> useless (LLaDA init)
        # =======================================================
    ...
    def setup_fsdp_sync(
        self, model: nn.Module, data_parallel: str, precision: str, grad_precision: Optional[str]
    ) -> FSDP:
