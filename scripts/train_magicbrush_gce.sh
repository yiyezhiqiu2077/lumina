...
: "${GCE_CLUSTER_PATH:?GCE_CLUSTER_PATH is required for objective=gce}"
...
echo "objective=gce config=$ROOT/configs/experiments/magicbrush_gce.yaml"
echo "MODEL_PATH=$MODEL_PATH DATA_ROOT=$DATA_ROOT DATA_CONFIG=$DATA_CONFIG OUTPUT_DIR=$OUTPUT_DIR GCE_CLUSTER_PATH=$GCE_CLUSTER_PATH"
...
"${TORCHRUN:-torchrun}" --standalone --nproc_per_node="${NPROC_PER_NODE:-4}" "$ROOT/train/train_magicbrush.py" \
  --objective gce --model "$MODEL_PATH" --train-manifest "$DATA_CONFIG" --output "$OUTPUT_DIR" \
  --gce-clusters "$GCE_CLUSTER_PATH" --gce-weight "${GCE_WEIGHT:-1.0}" --gce-levels 1024 512 \
